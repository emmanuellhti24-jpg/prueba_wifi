...
function setupSocketListeners() {
    socket.on('nuevo-pedido', (pedido) => {
        ...
        loadOrders();
        // Actualizar métricas en tiempo real si la pestaña está visible
        if (!document.getElementById('view-metrics').classList.contains('d-none')) {
            loadMetrics();
        }
    });
    socket.on('actualizacion-pedido', () => {
        loadOrders();
        // Actualizar métricas en tiempo real si la pestaña está visible
        if (!document.getElementById('view-metrics').classList.contains('d-none')) {
            loadMetrics();
        ...
        <tr>
            <td>
                <div class="d-flex flex-column align-items-center justify-content-center">
                    <img src="${p.imagen || 'https://via.placeholder.com/40'}" width="40" height="40" class="rounded mb-1" style="object-fit: cover;">
                    <span class="fw-bold small text-wrap" style="line-height: 1.2;">${p.nombre}</span>
                </div>
            </td>
    ...
    document.querySelector('#users-table tbody').innerHTML = usersList.map(u => `
        <tr>
            ...
            <td class="action-column">
                ...
                <button class="action-btn" onclick="deleteUser('${u._id}')">🗑️</button>
            </td>
        </tr>
    `).join('');
}
...
async function loadMetrics() {
    ...
    try {
        // OPTIMIZACIÓN: Carga paralela (4x más rápido)
        const [dashData, finData, catData, topData] = await Promise.all([
            fetch(`${API_URL}/metrics/dashboard${query}`, { headers }).then(r => r.json()),
            fetch(`${API_URL}/metrics/financials${query}`, { headers }).then(r => r.json()),
            fetch(`${API_URL}/metrics/categories${query}`, { headers }).then(r => r.json()),
            fetch(`${API_URL}/metrics/top-products${query}`, { headers }).then(r => r.json())
        ]);
        ...
        container.innerHTML = dashData.map(m => `
            <div class="col-md-3">
                <div class="metric-card">
                    ...
                    <div class="mt-2 badge bg-light text-dark text-wrap lh-sm w-100">${m.analisis}</div>
                </div>
            </div>
        `).join('');
        ...
        renderChart('financialChart', 'bar', {
            ...
            datasets: [{
                label: 'Total ($)',
                data: [finData.totalRevenue || 0, finData.totalCost || 0, finData.realProfit || 0],
                backgroundColor: ['#2ecc71', '#e74c3c', '#f1c40f']
            }]
        }, { plugins: { title: { display: true, text: 'Finanzas' } } });

        renderChart('categoryChart', 'doughnut', {
            labels: Array.isArray(catData) ? catData.map(d => (d.category || 'Otros').toUpperCase()) : [],
            datasets: [{
                data: Array.isArray(catData) ? catData.map(d => d.units) : [],
                backgroundColor: ['#e67e22', '#3498db', '#9b59b6', '#1abc9c']
            }]
        }, { plugins: { title: { display: true, text: 'Por Categoría' } } });

        renderChart('topProductsChart', 'bar', {
            labels: Array.isArray(topData) ? topData.map(d => d._id) : [],
            datasets: [{
                label: 'Unidades',
                data: Array.isArray(topData) ? topData.map(d => d.totalVendido) : [],
