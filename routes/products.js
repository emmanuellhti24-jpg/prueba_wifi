...
router.post('/', verificarToken, permitirRoles('admin'), upload.single('imagen'), async (req, res) => {
    try {
        ...
        const newProduct = new Product({
            id: `prod_${Date.now()}`, // Generar un ID único en el servidor
            nombre: datos.nombre,
...
router.put('/:id', verificarToken, permitirRoles('admin'), upload.single('imagen'), async (req, res) => {
    try {
        const datos = req.body;
        let imagenUrl = datos.imagenActual; // Mantener imagen anterior si no se sube una nueva
